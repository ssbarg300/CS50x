...
bool check(char *word)
{
    //declaring cursor to root
    node *cursor = root;
    //iterate through length of word
    for (int i = 0, n = strlen(word); i < n; i++)
    {
        //index is equal to the alphabetical number of these letters
        int index = tolower(word[i]) - 'a';

        //checks if children[index]isnt null
        if (cursor->children[index] != NULL)
        {
            //then sets cursor to it
            cursor = cursor->children[index];
            //if its a word then return true
            if (cursor->is_word == true)
            {
                return true;
            }
        }
        //else if cursor is equal to null and its not a word return false
        else if (cursor->children[index] == NULL && cursor->is_word == false)
        {
            return false;
        }
    }
